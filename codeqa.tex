...
Enforce a specific C++ standard by configuring the C++ compiler to fail it is violated.
We run unit tests for every commit, and generate code coverage and test statistics.
We issue verbose warnings, including Python's \texttt{DeprecationWarning}, to warn us of any possible future issues.
We run regular integration tests and tracking the algorithmic results to ensure that we are improving with time.
